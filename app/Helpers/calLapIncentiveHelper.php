...
class calLapIncentiveHelper
{
    ...
    public function calculateLapIncentive() 
    {
       ...
       if(empty($error)){
            ...
            ->whereRaw("CASE WHEN min = 1000000 and type='sales officer' THEN $this->disbursementAmt >= min and $this->disbursementAmt <=max WHEN min = 7500000 and type='portfolio manager' THEN $this->disbursementAmt >= min and $this->disbursementAmt <=max WHEN max = 0 THEN $this->disbursementAmt> min when max !=0 and min!=0 then $this->disbursementAmt> min and $this->disbursementAmt <=max END and type='$this->role'")
            ->get();
